...
public class MainWindow : Window, IDisposable
{
        ...
        try
        {
            if (ImGui.BeginTable("StatusTable", 5, ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingStretchSame))
            {
                ...
                foreach (var player in Plugin.ObjectTable.PlayerObjects)
                {
                    ...
                    {
                        playerDataList.Add(
                            new PlayerDataStructure(player, mastery, phantomJob!, resStacks, knowledgeLevel));
                    }
                }
                ...
                foreach (var playerData in playerDataList)
                {
                    ...
                    ImGui.TableNextColumn();
                    ImGui.Image(
                        Plugin.TextureProvider.GetFromGameIcon(62100u + player.ClassJob.RowId).GetWrapOrEmpty()
                              .Handle,
                        new Vector2(32, 32));
                    ImGui.SameLine();
    ...
    {
        return buffId is >= 4358 and <= 4369 or 4242;
    }

    private static bool IsResRestrictedDebuff(Status status)
    {
        // var iconId = status.GameData.Value.Icon;
        // return iconId is ResurrectionDeniedIconId or >= 218286 and <= 218288;
        return status.StatusId is 4262 or 4263;
    }
